...
void main(void)
{
	...
	c.x = (resolution.x / resolution.y) * (gl_FragCoord.x / resolution.x - 0.5) * scale;
	c.y = (gl_FragCoord.y / resolution.y - 0.5) * scale;
	c += offset.xy;
	...
	if (iters == 150) {
		...
		float center = mod(time / 10.0, 1.0);
		float m = float(iters) / 150.0;
		gl_FragColor = vec4(hsv2rgb(vec3(center + (upperMid * 0.5) * m, volume, m)), 1.0);
	}
}
